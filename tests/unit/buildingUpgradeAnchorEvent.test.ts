...
function buildUpgradeOnPlanet(planetId: number, startedAt: number, durationMs: number, buildingType: number): PlayerDataType.BuildingUpgrade
{
...
describe('findNextAnchorEvent (building upgrade)', () =>
{
    it('returns null when no upgrades exist', () =>
    {
        ...
        const result: AnchorEvent.AnchorEvent | null = BuildingUpgradeAnchorEvent.findNextAnchorEvent(playerData);
        expect(result).toBeNull();
    });

    it('returns null for an upgrade that has not been started', () =>
    {
        const notStarted: PlayerDataType.BuildingUpgrade =
        {
    ...
    it('returns the anchor event with the correct completion time', () =>
    {
        ...
        const upgrade: PlayerDataType.BuildingUpgrade = buildUpgradeOnPlanet(1, startedAt, durationMs, GameType.BUILDING_RESOURCE_PRODUCTION_1);
    ...
    {
        const earlyUpgrade: PlayerDataType.BuildingUpgrade = buildUpgradeOnPlanet(1, 1_000_000, 5_000, GameType.BUILDING_RESOURCE_PRODUCTION_1);
        const lateUpgrade: PlayerDataType.BuildingUpgrade = buildUpgradeOnPlanet(2, 1_000_000, 20_000, GameType.BUILDING_RESOURCE_PRODUCTION_2);
...
describe('resolveAnchorEvent (building upgrade)', () =>
{
    ...
    {
        const upgrade: PlayerDataType.BuildingUpgrade = buildUpgradeOnPlanet(1, 1_000_000, 30_000, GameType.BUILDING_RESOURCE_PRODUCTION_1);
        ...
        const anchorEventResult: AnchorEvent.AnchorEvent | null = BuildingUpgradeAnchorEvent.findNextAnchorEvent(playerData);
    ...
    it('removes the upgrade from the planet after resolution', () =>
    {
        ...
        expect(playerData.fullPlanetDatas[0]!.dynamicPlanetData.buildingUpgrades).toHaveLength(0);
    });
});
