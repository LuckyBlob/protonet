...
describe('computeFleetMovementDurationSecondsFromAddresses', () =>
{
    it('returns a positive duration for a non-zero distance with a Small Transport', () =>
    {
        ...
        const duration: number = FleetMovementDuration.computeFleetMovementDurationSecondsFromAddresses(PLAYER_DATA, ORIGIN_SAME, TARGET_SLOT_DIFF, shipQuantities, null);
    ...
    {
        const shipQuantities: Map<GameType.ShipType, number> = new Map([[GameType.ShipType.SmallTransport, 1]]);
        const duration: number = FleetMovementDuration.computeFleetMovementDurationSecondsFromAddresses(PLAYER_DATA, ORIGIN_SAME, ORIGIN_SAME, shipQuantities, null);
        // 10 + 3500*sqrt(0) = 10
    ...
    {
        const shipQuantities: Map<GameType.ShipType, number> = new Map([[GameType.ShipType.SmallTransport, 1]]);
        const slotDiff: number = FleetMovementDuration.computeFleetMovementDurationSecondsFromAddresses(PLAYER_DATA, ORIGIN_SAME, TARGET_SLOT_DIFF, shipQuantities, null);
        const systemDiff: number = FleetMovementDuration.computeFleetMovementDurationSecondsFromAddresses(PLAYER_DATA, ORIGIN_SAME, TARGET_SYSTEM_DIFF, shipQuantities, null);
        const galaxyDiff: number = FleetMovementDuration.computeFleetMovementDurationSecondsFromAddresses(PLAYER_DATA, ORIGIN_SAME, TARGET_GALAXY_DIFF, shipQuantities, null);
    ...
    it('applies time_multiplier from serverData (2× yields half the duration, floored)', () =>
    {
        ...
        const base: number = FleetMovementDuration.computeFleetMovementDurationSecondsFromAddresses(PLAYER_DATA, ORIGIN_SAME, TARGET_SYSTEM_DIFF, shipQuantities, null);
        const accelerated: number = FleetMovementDuration.computeFleetMovementDurationSecondsFromAddresses(PLAYER_DATA, ORIGIN_SAME, TARGET_SYSTEM_DIFF, shipQuantities, serverData);
...
describe('computeFleetMovementDurationSecondsWithAddress', () =>
{
    ...
    {
        const shipQuantities: Map<GameType.ShipType, number> = new Map([[GameType.ShipType.SmallTransport, 1]]);
        const fromAddresses: number = FleetMovementDuration.computeFleetMovementDurationSecondsFromAddresses(PLAYER_DATA, ORIGIN_SAME, TARGET_SYSTEM_DIFF, shipQuantities, null);
        const withAddress: number = FleetMovementDuration.computeFleetMovementDurationSecondsWithAddress(PLAYER_DATA, ORIGIN_SAME, TARGET_SYSTEM_DIFF, shipQuantities, null);
...
describe('computeFleetMovementDurationSeconds (planet form)', () =>
{
    it('agrees with the address-based form for the same coordinates', () =>
    {
        ...
        const planetForm: number = FleetMovementDuration.computeFleetMovementDurationSeconds(PLAYER_DATA, origin, target, shipQuantities, null);
        const addressForm: number = FleetMovementDuration.computeFleetMovementDurationSecondsFromAddresses(PLAYER_DATA, ORIGIN_SAME, TARGET_SYSTEM_DIFF, shipQuantities, null);
