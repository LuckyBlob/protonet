...
describe('serializePlayerData / deserializePlayerData', () =>
{
    ...
    {
        const original: PlayerDataType.PlayerData = TestDataBuilders.buildPlayerData();
        const serialized: Serialization.SerializedPlayerData = Serialization.serializePlayerData(original);
        const restored: PlayerDataType.PlayerData = Serialization.deserializePlayerData(serialized);

        expect(restored.playerRow.id).toBe(original.playerRow.id);
        expect(restored.fullPlanetDatas).toHaveLength(original.fullPlanetDatas.length);

        const originalResources: Map<number, number> = original.fullPlanetDatas[0]!.dynamicPlanetData.resourceQuantity;
        const restoredResources: Map<number, number> = restored.fullPlanetDatas[0]!.dynamicPlanetData.resourceQuantity;
    ...
    {
        const fullPlanetData: PlayerDataType.FullPlanetData = TestDataBuilders.buildFullPlanetData(
        {
            dynamicPlanetData:
            {
        ...
        const original: PlayerDataType.PlayerData = TestDataBuilders.buildPlayerData({ fullPlanetDatas: [fullPlanetData] });
        const serialized: Serialization.SerializedPlayerData = Serialization.serializePlayerData(original);
        const restored: PlayerDataType.PlayerData = Serialization.deserializePlayerData(serialized);

        const restoredLevels: Map<number, number> = restored.fullPlanetDatas[0]!.dynamicPlanetData.buildingLevels;
    ...
    it('defaults buildingUpgrades to empty array when field is absent in wire data', () =>
    {
        ...
        delete (withoutUpgrades.fullPlanetDatas[0]!.dynamicPlanetData as any).buildingUpgrades;

        const restored: PlayerDataType.PlayerData = Serialization.deserializePlayerData(withoutUpgrades);
        expect(restored.fullPlanetDatas[0]!.dynamicPlanetData.buildingUpgrades).toEqual([]);
    });
});
