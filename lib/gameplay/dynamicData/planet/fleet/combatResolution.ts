...
{
    combatResult: CombatResolver.CombatResult;
    ...
    numRounds: number;
    attackerWon: boolean;
    attackerSurvivingUnitTotal: number;
};

// Deliberate order shared by Attack and Destroy Moon: fight -> debris + wreck -> defense repair -> plunder (win judged on combat result, not post-repair defenses).
export function resolveBattleAndAftermath(originPlayerData: CoreType.PlayerData, targetPlayerData: CoreType.PlayerData, aimedBody: CoreType.PlanetData, targetAddress: GameType.PlanetAddress, fleetMovement: CoreType.FleetMovement): BattleAftermath
{
    ...
    const defenderSurvivingCombatTotal: number = MathHelp.calculateTotalQuantityMap(combatResult.defenderUnitQuantities);
    const attackerWon: boolean = attackerSurvivingUnitTotal > 0 && defenderSurvivingCombatTotal === 0;
    ...
    const battleAftermath: BattleAftermath =
    {
        ...
        attackerSurvivingUnitTotal: attackerSurvivingUnitTotal,
    };

    return battleAftermath;
}

export function buildDefenderCombatUnitQuantities(aimedBody: CoreType.PlanetData): Map<GameType.UnitType, number>
{
...
export function rewriteAttackerFleetUnitRows(fleetMovement: CoreType.FleetMovement, survivingUnitQuantities: Map<GameType.UnitType, number>): void
{
...
export function createDefenderOwnedZone(targetPlayerData: CoreType.PlayerData, zoneAddress: GameType.PlanetAddress, size: number, claimedAt: number): CoreType.PlanetData
{
...
export function buildBattleSummaryLines(publicPlayerDatas: CoreType.PublicPlayerData[], fleetRow: DBType.FleetMovementRow, aftermath: BattleAftermath): string[]
{
    ...
    reportLines.push(`Attacker losses: ${FleetData.buildUnitQuantitiesList(aftermath.attackerLosses, "none")}`);
    reportLines.push(`Defender losses: ${FleetData.buildUnitQuantitiesList(aftermath.defenderLosses, "none")}`);
    reportLines.push(`Defenses rebuilt: ${FleetData.buildUnitQuantitiesList(aftermath.repairedDefenseQuantities, "none")}`);
    reportLines.push(`Resources captured: ${FleetData.buildResourceQuantitiesList(aftermath.lootedResourceQuantities)}`);
    reportLines.push(`Debris field: ${FleetData.buildResourceQuantitiesList(aftermath.debrisResourceQuantities)}`);

    if (aftermath.wreckUnitQuantities.size > 0)
    {
        reportLines.push(`Wreck field (repairable at Repair Dock): ${FleetData.buildUnitQuantitiesList(aftermath.wreckUnitQuantities, "none")}`);
    }
